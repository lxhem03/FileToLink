...
def srt_to_webvtt(srt_bytes: bytes) -> bytes:
    """Convert SRT bytes to strict WebVTT bytes that Chrome accepts."""
    import re as _re
    try:
...
async def info_handler(request: web.Request):
    try:
        ...
        fc    = multi_clients[index]
        if fc not in class_cache:
            class_cache[fc] = ByteStreamer(fc)
        file_id = await class_cache[fc].get_file_properties(id)
        ...
        cmd = ["ffprobe", "-v", "quiet", "-print_format", "json",
               "-show_streams", file_url]
        proc = await asyncio.create_subprocess_exec(
            *cmd, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE)
        try:
            stdout, _ = await asyncio.wait_for(proc.communicate(), timeout=30)
        except asyncio.TimeoutError:
            return web.json_response({"audio": [], "subtitles": [], "error": "timeout"},
        ...
        if stdout:
            try:
                ...
                for s in _json.loads(stdout).get("streams", []):
                    ...
                    if ct == "audio":
                        audio.append({"index": ai, "label": title or f"Audio {ai+1}", "lang": lang})
                        ai += 1
                    elif ct == "subtitle":
                        codec = s.get("codec_name", "")
                        if codec in ("subrip","srt","ass","ssa","webvtt","mov_text","hdmv_pgs_subtitle"):
                            subs.append({"index": si, "label": title or f"Subtitle {si+1}",
                                         "lang": lang, "codec": codec})
                        si += 1
            except Exception as e:
                logging.warning(f"ffprobe parse: {e}")
        ...
        logging.error(f"info_handler: {e}")
...
async def audio_handler(request: web.Request):
    try:
        ...
        secure_hash = request.rel_url.query.get("hash", "")
        try:
            start_sec = max(0.0, float(request.rel_url.query.get("start", "0") or "0"))
        ...
        logging.error(f"audio_handler: {e}")
        raise web.HTTPInternalServerError(text=str(e))


# ── /sub/{id}/{track}?hash= — subtitle as WebVTT ─────────────────────────────
_sub_cache      = {}
_sub_codec_cache= {}
...
async def sub_handler(request: web.Request):
    try:
        id          = int(request.match_info["id"])
        ti          = int(request.match_info["track"])
        secure_hash = request.rel_url.query.get("hash", "")
        ...
        if ck in _sub_cache:
            ...
            return web.Response(
                body=data, content_type="text/vtt", charset="utf-8",
                headers={"Access-Control-Allow-Origin":  "*",
        ...
        if not codec:
            try:
                ...
                if ti < len(streams):
                    codec = streams[ti].get("codec_name", "")
            except Exception:
                codec = ""
        ...
        # Extract as SRT then convert to strict WebVTT in Python
        cmd = ["ffmpeg", "-nostdin", "-probesize", "50M", "-analyzeduration", "0",
               "-i", file_url, "-map", f"0:s:{ti}", "-f", "srt", "pipe:1"]
        proc = await asyncio.create_subprocess_exec(
            *cmd, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE)
        try:
        ...
        if proc.returncode != 0 or not stdout:
            ...
            logging.warning(f"sub fail id={id} t={ti}: {err}")
            raise web.HTTPNotFound(text=f"Subtitle failed: {err[:100]}")

        vtt = srt_to_webvtt(stdout)
        _sub_cache[ck]       = (vtt, fmt)
        _sub_codec_cache[ck] = codec

        return web.Response(
            body=vtt, content_type="text/vtt", charset="utf-8",
            headers={"Access-Control-Allow-Origin":  "*",
                     "Access-Control-Allow-Headers": "*",
                     "X-Subtitle-Format":            fmt,
                     "Cache-Control":                "public, max-age=86400"})
        ...
        logging.error(f"sub_handler: {e}", exc_info=True)
        raise web.HTTPInternalServerError(text=str(e))


# ── /{path} — original file streamer (Range-request, fully seekable) ──────────
